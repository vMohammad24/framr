...
pub fn convert_format(f: WlFormat) -> Option<PixelFormat> {
	match f {
		...
		WlFormat::Abgr8888 => Some(PixelFormat::Abgr8888),
		WlFormat::Xbgr8888 => Some(PixelFormat::Xbgr8888),
		WlFormat::Abgr2101010 => Some(PixelFormat::Abgr2101010),
		WlFormat::Xbgr2101010 => Some(PixelFormat::Xbgr2101010),
		_ => None,
	}
}
	...
	fn event(
		...
		match event {
			FrameEvent::Buffer {
				...
				let Ok(fmt) = format.into_result() else {
					return;
				};
				if let Some(pixel_fmt) = convert_format(fmt) {
