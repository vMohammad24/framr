...
impl ToolBehavior for HighlightTool {
	...
	fn keys(&self) -> Vec<Keysym> {
		vec![Keysym::_7, Keysym::h, Keysym::H]
	}
	...
	fn apply_effect(
		...
		if bw > 0 && bh > 0 {
			graphics::apply_highlight(img, bx, by, bw, bh, config.highlight_color);
		}
	}
	...
	fn render(
		...
		if let Some((x, y, w, h)) = region_rect(ann, offset_x, offset_y) {
			graphics::set_source_color(cr, config.highlight_color);
