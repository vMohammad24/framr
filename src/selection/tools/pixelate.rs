...
impl ToolBehavior for PixelateTool {
	...
	fn keys(&self) -> Vec<Keysym> {
		vec![Keysym::_6, Keysym::p, Keysym::P]
	}
	...
	fn apply_effect(
		...
		if bw > 0 && bh > 0 {
			graphics::apply_pixelate(img, bx, by, bw, bh, config.pixelate_block_size);
		}
	}
	...
	fn render(
		...
		if let Some((x, y, w, h)) = region_rect(ann, offset_x, offset_y) {
			if let Err(e) = cr.set_source_surface(&surface_data.cached_pixelated_bg, 0.0, 0.0) {
